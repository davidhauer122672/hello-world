...
// ── GET /v1/franchise/config ────────────────────────────────────────────────

/**
 * Return the white-label franchise configuration template.
 *
 * Provides a complete template that a prospective franchise operator
 * would customize before provisioning. Includes brand settings,
 * available territories, tier options, compliance info, and feature flags.
 *
 * @param {Request} request
 * @param {object} env — Cloudflare Worker environment bindings
 * @param {object} ctx — Execution context (waitUntil)
 * @returns {Response} JSON response with franchise configuration template
 */
export function handleFranchiseConfig(request, env, ctx) {
  ...
  const config = {
    ...
    featureFlags: {
      enableRetellVoice: false,
      enableInvestorPipeline: false,
      enableVideoProduction: false,
      enablePodcastProduction: false,
      enableApiMarketplace: false,
      enableWhiteLabelPortal: true,
      enableDynamicPricing: false,
      enableCallAnalytics: false,
      enableAdvancedReporting: false,
      enableConstantContactIntegration: true,
...
// ── POST /v1/franchise/provision ────────────────────────────────────────────

/**
 * Provision a new franchise territory.
 *
 * Validates the requested territory and zones, checks for overlap with
 * existing claimed territories, and returns a provisioning plan for
 * approval. Does NOT create infrastructure — the returned plan must
 * be reviewed and approved before execution.
 *
 * Body:
 *   franchiseName (string, required) — Operator's brand name
 *   territory     (string, required) — Territory key from SERVICE_ZONES
 *   zones         (string[], required) — Specific zones within the territory to claim
 *   contactEmail  (string, required) — Primary contact email for the franchise
 *   tier          (string, required) — "starter" | "growth" | "enterprise"
 *
 * @param {Request} request
 * @param {object} env — Cloudflare Worker environment bindings
 * @param {object} ctx — Execution context (waitUntil)
 * @returns {Response} JSON response with provisioning plan
 */
export async function handleFranchiseProvision(request, env, ctx) {
  ...
  if (!territory || typeof territory !== 'string') {
    return errorResponse('"territory" is required.', 400);
  }

  if (!Array.isArray(zones) || zones.length === 0) {
    return errorResponse('"zones" must be a non-empty array of zone names.', 400);
  }

  if (!contactEmail || typeof contactEmail !== 'string' || !contactEmail.includes('@')) {
    return errorResponse('"contactEmail" must be a valid email address.', 400);
  }
  ...
  const territoryEntry = Object.entries(SERVICE_ZONES).find(
    ([, data]) => data.territory === territory
  );

  if (!territoryEntry) {
    const validTerritories = Object.values(SERVICE_ZONES).map(d => d.territory);
    return errorResponse(
      `Unknown territory "${territory}". Valid territories: ${validTerritories.join(', ')}.`,
      400,
    );
  }
  ...
  if (territoryData.availability === 'claimed') {
    return errorResponse(
      `Territory "${territoryName}" is already claimed by ${territoryData.operator}. ` +
      'Contact franchise@coastalkey-pm.com for sub-licensing options.',
      409,
    );
  }

  // ── Validate requested zones belong to territory ──

  const invalidZones = zones.filter(z => !territoryData.zones.includes(z));
  if (invalidZones.length > 0) {
    return errorResponse(
      `Zones not found in territory "${territoryName}": ${invalidZones.join(', ')}. ` +
      `Available zones: ${territoryData.zones.join(', ')}.`,
      400,
    );
  }

  // ── Check zone count against tier limit ──

  const tierConfig = FRANCHISE_TIERS[tier];

  if (zones.length > tierConfig.maxZones) {
    return errorResponse(
      `Tier "${tierConfig.name}" allows a maximum of ${tierConfig.maxZones} zones. ` +
      `Requested: ${zones.length}. Upgrade to a higher tier or reduce zone selection.`,
      400,
    );
  }

  // ── Check for zone overlap with all claimed territories ──
  ...
  if (overlappingZones.length > 0) {
    return errorResponse(
      `Zone overlap detected. The following zones are already claimed: ${overlappingZones.join(', ')}.`,
      409,
    );
  }
  ...
  const estimatedMonthlyCost =
    tierConfig.monthlyBase +
    (zones.length * 150); // per-zone infrastructure overhead
  ...
  const provisioningPlan = {
    ...
    zones: {
      requested: zones,
      count: zones.length,
      maxAllowed: tierConfig.maxZones,
      remaining: tierConfig.maxZones - zones.length,
    },
    ...
    onboarding: {
      ...
      includes: [
        'Platform configuration and branding setup',
        'Agent training and customization',
        'CRM data migration assistance',
    ...
    compliance: COMPLIANCE_REQUIREMENTS,
    nextSteps: [
      'Review this provisioning plan with your team',
      'Sign the Coastal Key Franchise Agreement',
      'Submit CAM license verification for your territory',
      'Complete onboarding payment ($5,000 one-time setup)',
      'Schedule kickoff call with your dedicated onboarding specialist',
    ],
  ...
  writeAudit(env, ctx, {
    route: '/v1/franchise/provision',
    action: 'provision_plan_created',
...
// ── GET /v1/franchise/territories ───────────────────────────────────────────

/**
 * List all defined franchise territories with their zones and availability.
 *
 * Returns SERVICE_ZONES grouped by territory. Each entry includes the
 * territory name, zone list, availability status, and current operator
 * (if claimed).
 *
 * @param {Request} request
 * @param {object} env — Cloudflare Worker environment bindings
 * @param {object} ctx — Execution context (waitUntil)
 * @returns {Response} JSON response with territories array
 */
export function handleFranchiseTerritories(request, env, ctx) {
  ...
  writeAudit(env, ctx, {
    route: '/v1/franchise/territories',
    action: 'territories_listed',
  });
