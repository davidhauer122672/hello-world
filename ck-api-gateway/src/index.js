 ...
 *   POST /v1/mcco/audience-psychology — Audience psychology breakdown
 *   POST /v1/mcco/positioning  — Brand positioning strategy
 *   POST /v1/mcco/content-calendar — 30-day content calendar
 *   POST /v1/mcco/social-post  — High-engagement social post generator
 *   POST /v1/mcco/monetization — Monetization strategy
...
import { handleMCCOStatus, handleMCCOBrand, handleMCCOPillars, handleMCCOPipeline, handleAudiencePsychology, handlePositioning, handleContentCalendar, handleSocialPost, handleMonetization, handleMCCODashboard } from './routes/mcco.js';
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/v1/mcco/status' && method === 'GET') {
        return handleMCCOStatus();
      }

      if (path === '/v1/mcco/brand' && method === 'GET') {
        return handleMCCOBrand();
      }

      if (path === '/v1/mcco/pillars' && method === 'GET') {
        return handleMCCOPillars();
      }

      if (path === '/v1/mcco/pipeline' && method === 'GET') {
        return handleMCCOPipeline();
      }

      if (path === '/v1/mcco/dashboard' && method === 'GET') {
        return handleMCCODashboard();
      }

      if (path === '/v1/mcco/audience-psychology' && method === 'POST') {
        return await handleAudiencePsychology(request, env, ctx);
      }

      if (path === '/v1/mcco/positioning' && method === 'POST') {
        return await handlePositioning(request, env, ctx);
      }

      if (path === '/v1/mcco/content-calendar' && method === 'POST') {
        return await handleContentCalendar(request, env, ctx);
      }

      if (path === '/v1/mcco/social-post' && method === 'POST') {
        return await handleSocialPost(request, env, ctx);
      }

      if (path === '/v1/mcco/monetization' && method === 'POST') {
        return await handleMonetization(request, env, ctx);
      }
