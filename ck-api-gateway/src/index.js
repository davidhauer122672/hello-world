  ...
  async fetch(request, env, ctx) {
    ...
    if (path === '/v1/health' && method === 'GET') {
      ...
      if (env.ATLAS_API_KEY) {
        ...
        } catch (err) { checks.atlas = { status: 'error', platform: 'youratlas.com', message: err.message }; }
      } else { checks.atlas = { status: 'not_configured', platform: 'youratlas.com' }; }
      checks.kv = { cache: env.CACHE ? 'available' : 'missing', sessions: env.SESSIONS ? 'available' : 'missing', rateLimits: env.RATE_LIMITS ? 'available' : 'missing', auditLog: env.AUDIT_LOG ? 'available' : 'missing' };
      const allOk = checks.airtable?.status === 'ok' && checks.anthropic?.status === 'ok' && checks.kv.cache === 'available';
      return jsonResponse({ status: allOk ? 'operational' : 'degraded', service: 'ck-api-gateway', version: '2.0.0', agents: 312, divisions: 10, checks, timestamp: new Date().toISOString() });
    }

    // ── Public routes (no auth) ──
    if (path === '/v1/leads/public' && method === 'POST') { return await handlePublicLead(request, env, ctx); }

    // ── Slack routes (use signature verification, not Bearer token) ──
    if (path === '/v1/slack/commands' && method === 'POST') { return await handleSlackCommand(request, env, ctx); }
    if (path === '/v1/slack/interactions' && method === 'POST') { return await handleSlackInteraction(request, env, ctx); }
    if (path === '/v1/slack/events' && method === 'POST') { return await handleSlackEvent(request, env, ctx); }
    ...
    try {
      // ── Route dispatch ──
      if (path === '/v1/inference' && method === 'POST') { return await handleInference(request, env, ctx); }
      if (path === '/v1/leads' && method === 'POST') { return await handleCreateLead(request, env, ctx); }
      if (path === '/v1/leads/enrich' && method === 'POST') { return await handleEnrichLead(request, env, ctx); }
      if (path.startsWith('/v1/leads/') && method === 'GET') { const recordId = path.split('/v1/leads/')[1]; return await handleGetLead(recordId, env); }
      if (path === '/v1/webhook/retell' && method === 'POST') { return await handleRetellWebhook(request, env, ctx); }
      if (path === '/v1/content/generate' && method === 'POST') { return await handleContentGenerate(request, env, ctx); }
      if (path === '/v1/agents' && method === 'GET') { return handleListAgents(url, env); }
      if (path === '/v1/agents/metrics' && method === 'GET') { return handleAgentMetrics(url, env); }
      if (path === '/v1/dashboard' && method === 'GET') { return await handleDashboard(env); }
      if (path.match(/^\/v1\/agents\/[^/]+\/action$/) && method === 'POST') { return await handleAgentAction(request, env, ctx); }
      if (path.match(/^\/v1\/agents\/[^/]+$/) && method === 'GET') { const agentId = path.split('/v1/agents/')[1]; return handleGetAgent(agentId, env); }

      // ── Workflow Pipelines ──
      if (path === '/v1/workflows/scaa1' && method === 'POST') { return await handleScaa1BattlePlan(request, env, ctx); }
      if (path === '/v1/workflows/wf3' && method === 'POST') { return await handleWf3InvestorEscalation(request, env, ctx); }
      if (path === '/v1/workflows/wf4' && method === 'POST') { return await handleWf4LongTailNurture(request, env, ctx); }

      // ── Property Intelligence ──
      if (path === '/v1/property-intel/search' && method === 'GET') { return await handlePropertySearch(url, env); }
      if (path === '/v1/property-intel/import' && method === 'POST') { return await handlePropertyImport(request, env, ctx); }
      if (path === '/v1/property-intel/stats' && method === 'GET') { return await handlePropertyStats(env); }

      // ── TH Sentinel Campaign ──
      if (path === '/v1/campaign/calls' && method === 'GET') { return await handleCampaignCallLog(url, env); }
      if (path === '/v1/campaign/agents' && method === 'GET') { return await handleCampaignAgentPerformance(url, env); }
      if (path === '/v1/campaign/analytics' && method === 'GET') { return await handleCampaignAnalytics(url, env); }
      if (path === '/v1/campaign/contacts' && method === 'GET') { return await handleCampaignLeadContacts(url, env); }
      if (path === '/v1/campaign/dashboard' && method === 'GET') { return await handleCampaignDashboard(env); }

      // ── Pricing Engine ──
      if (path === '/v1/pricing/recommend' && method === 'POST') { return await handlePricingRecommend(request, env, ctx); }
      if (path === '/v1/pricing/zones' && method === 'GET') { return handlePricingZones(); }

      // ── Intelligence Officers ──
      if (path === '/v1/intel/officers' && method === 'GET') { return handleListOfficers(url); }
      if (path === '/v1/intel/dashboard' && method === 'GET') { return await handleOfficerDashboard(env); }
      if (path === '/v1/intel/fleet-scan' && method === 'POST') { return await handleFleetScan(env, ctx); }
      if (path.match(/^\/v1\/intel\/officers\/[^/]+\/scan$/) && method === 'POST') { return await handleOfficerScan(request, env, ctx); }
      if (path.match(/^\/v1\/intel\/officers\/[^/]+$/) && method === 'GET') { const officerId = path.split('/v1/intel/officers/')[1]; return handleGetOfficer(officerId); }

      // ── Email Agents ──
      if (path === '/v1/email/agents' && method === 'GET') { return handleListEmailAgents(url); }
      if (path === '/v1/email/dashboard' && method === 'GET') { return handleEmailDashboard(); }
      if (path === '/v1/email/compose' && method === 'POST') { return await handleEmailCompose(request, env, ctx); }
      if (path === '/v1/email/classify' && method === 'POST') { return await handleEmailClassify(request, env, ctx); }
      if (path.match(/^\/v1\/email\/agents\/[^/]+$/) && method === 'GET') { const agentId = path.split('/v1/email/agents/')[1]; return handleGetEmailAgent(agentId); }
      if (path === '/v1/audit' && method === 'GET') { return await handleAuditLog(url, env); }

      // ── MCCO — Sovereign Governance ──
      if (path === '/v1/mcco/command' && method === 'GET') { return handleMCCOCommand(); }
      if (path === '/v1/mcco/agents' && method === 'GET') { return handleListMCCOAgents(url); }
      if (path === '/v1/mcco/fleet-status' && method === 'GET') { return handleMCCOFleetStatus(); }
      if (path === '/v1/mcco/directive' && method === 'POST') { return await handleMCCODirective(request, env, ctx); }
      if (path === '/v1/mcco/content-calendar' && method === 'POST') { return await handleMCCOContentCalendar(request, env, ctx); }
      if (path === '/v1/mcco/audience-profile' && method === 'POST') { return await handleMCCOAudienceProfile(request, env, ctx); }
      if (path === '/v1/mcco/positioning' && method === 'POST') { return await handleMCCOPositioning(request, env, ctx); }
      if (path === '/v1/mcco/monetization' && method === 'POST') { return await handleMCCOMonetization(request, env, ctx); }
      if (path === '/v1/mcco/post' && method === 'POST') { return await handleMCCOPost(request, env, ctx); }
      if (path.match(/^\/v1\/mcco\/agents\/[^/]+$/) && method === 'GET') { const agentId = path.split('/v1/mcco/agents/')[1]; return handleGetMCCOAgent(agentId); }
      if (path === '/v1/mcco/master-plan' && method === 'GET') { return handleMasterPlan(); }
      if (path.match(/^\/v1\/mcco\/master-plan\/phase\/[^/]+$/) && method === 'GET') { const phaseId = path.split('/v1/mcco/master-plan/phase/')[1]; return handleMasterPlanPhase(phaseId); }
      if (path.match(/^\/v1\/mcco\/master-plan\/division\/[^/]+$/) && method === 'GET') { const divisionId = path.split('/v1/mcco/master-plan/division/')[1]; return handleDivisionPlan(divisionId); }
      if (path === '/v1/mcco/sovereign-directive' && method === 'POST') { return await handleSovereignDirectiveIssue(request, env, ctx); }
      if (path === '/v1/mcco/activation-status' && method === 'GET') { return handleActivationStatus(); }

      // ── Atlas AI Campaign Platform ──
      if (path === '/v1/atlas/health' && method === 'GET') { return await handleAtlasHealth(env); }
      if (path === '/v1/atlas/campaigns' && method === 'GET') { return await handleAtlasCampaigns(env); }
      if (path === '/v1/atlas/campaigns' && method === 'POST') { return await handleAtlasCreateCampaign(request, env, ctx); }
      if (path === '/v1/atlas/audit' && method === 'GET') { return await handleAtlasAudit(env); }
      if (path === '/v1/atlas/statistics' && method === 'GET') { return await handleAtlasOverviewStats(env); }
      if (path === '/v1/atlas/speed-to-lead' && method === 'POST') { return await handleAtlasSpeedToLead(request, env, ctx); }
      if (path === '/v1/atlas/kb/files' && method === 'GET') { return await handleAtlasKBFiles(env); }
      if (path.match(/^\/v1\/atlas\/campaigns\/[^/]+\/status$/) && method === 'PUT') { const campaignId = path.split('/v1/atlas/campaigns/')[1].replace('/status', ''); return await handleAtlasCampaignStatus(request, campaignId, env, ctx); }
      if (path.match(/^\/v1\/atlas\/campaigns\/[^/]+\/setup-revival$/) && method === 'POST') { const campaignId = path.split('/v1/atlas/campaigns/')[1].replace('/setup-revival', ''); return await handleAtlasSetupRevival(campaignId, env, ctx); }
      if (path.match(/^\/v1\/atlas\/campaigns\/[^/]+\/stats$/) && method === 'GET') { const campaignId = path.split('/v1/atlas/campaigns/')[1].replace('/stats', ''); return await handleAtlasCampaignStatsById(campaignId, env); }
      if (path.match(/^\/v1\/atlas\/campaigns\/[^/]+\/calls\/[^/]+$/) && method === 'GET') { const parts = path.match(/^\/v1\/atlas\/campaigns\/([^/]+)\/calls\/([^/]+)$/); return await handleAtlasCallRecordDetail(parts[1], parts[2], env); }
      if (path.match(/^\/v1\/atlas\/campaigns\/[^/]+\/calls$/) && method === 'GET') { const campaignId = path.split('/v1/atlas/campaigns/')[1].replace('/calls', ''); return await handleAtlasCallRecords(campaignId, url, env); }
      if (path.match(/^\/v1\/atlas\/campaigns\/[^/]+\/schedule$/) && method === 'POST') { const campaignId = path.split('/v1/atlas/campaigns/')[1].replace('/schedule', ''); return await handleAtlasScheduleCall(request, campaignId, env, ctx); }
      if (path.match(/^\/v1\/atlas\/campaigns\/[^/]+\/bookings$/) && method === 'GET') { const campaignId = path.split('/v1/atlas/campaigns/')[1].replace('/bookings', ''); return await handleAtlasCampaignBookings(campaignId, env); }
      if (path.match(/^\/v1\/atlas\/campaigns\/[^/]+$/) && method === 'GET') { const campaignId = path.split('/v1/atlas/campaigns/')[1]; return await handleAtlasCampaignById(campaignId, env); }

      // ── Peak Performance Frameworks ──
      if (path === '/v1/frameworks' && method === 'GET') { return handleListFrameworks(url); }
      if (path === '/v1/frameworks/apply' && method === 'POST') { return await handleFrameworkApply(request, env, ctx); }
      if (path === '/v1/frameworks/content' && method === 'POST') { return await handleFrameworkContent(request, env, ctx); }
      if (path === '/v1/frameworks/sales-playbook' && method === 'POST') { return await handleFrameworkSalesPlaybook(request, env, ctx); }
      if (path === '/v1/frameworks/productivity-plan' && method === 'POST') { return await handleFrameworkProductivityPlan(request, env, ctx); }
      if (path.match(/^\/v1\/frameworks\/category\/[^/]+$/) && method === 'GET') { const category = path.split('/v1/frameworks/category/')[1]; return handleGetFrameworksByCategory(category); }
      if (path.match(/^\/v1\/frameworks\/[^/]+$/) && method === 'GET') { const frameworkId = path.split('/v1/frameworks/')[1]; return handleGetFramework(frameworkId); }
      ...
      if (path === '/v1/financial/forecast' && method === 'POST') { return await handleFinancialForecast(request); }
      if (path === '/v1/financial/pricing-strategy' && method === 'POST') { return await handleDynamicPricing(request); }
      if (path === '/v1/financial/budget' && method === 'POST') { return await handleBudget(request); }

      // ── Analysis Suite ──
      if (path === '/v1/analysis/agent' && method === 'POST') { return await handleAgentAnalysis(request); }
      if (path === '/v1/analysis/fleet' && method === 'POST') { return await handleFleetAnalytics(request); }
      if (path === '/v1/analysis/market-trends' && method === 'POST') { return await handleMarketTrends(request); }
      if (path === '/v1/analysis/competitive-intel' && method === 'POST') { return await handleCompetitiveIntel(request); }
      if (path === '/v1/analysis/lead-pipeline' && method === 'POST') { return await handleLeadPipeline(request); }
      if (path === '/v1/analysis/operational-report' && method === 'POST') { return await handleOperationalReport(request); }
      if (path === '/v1/analysis/templates' && method === 'GET') { return handleAnalysisTemplates(); }
      if (path === '/v1/analysis/property-health' && method === 'POST') { return await handlePropertyHealth(request); }
      if (path === '/v1/analysis/churn-prediction' && method === 'POST') { return await handleChurnPrediction(request); }

      // ── Trading / Deal Engine ──
      if (path === '/v1/deals/stages' && method === 'GET') { return handleDealStages(); }
      if (path === '/v1/deals/score' && method === 'POST') { return await handleScoreDeal(request); }
      if (path === '/v1/deals/strategy' && method === 'POST') { return await handleDealStrategy(request); }
      if (path === '/v1/deals/comparables' && method === 'POST') { return await handleComparables(request); }
      if (path === '/v1/deals/closing-costs' && method === 'POST') { return await handleClosingCosts(request); }
      if (path === '/v1/deals/investor-package' && method === 'POST') { return await handleInvestorPackage(request); }
      if (path === '/v1/deals/portfolio' && method === 'POST') { return await handlePortfolioEvaluation(request); }

      // ── AI Trader Agent ──
      if (path === '/v1/trader/dashboard' && method === 'GET') { return await handleTraderDashboard(env); }
      if (path === '/v1/trader/agent' && method === 'GET') { return handleTraderAgent(); }
      if (path === '/v1/trader/watchlist' && method === 'GET') { return handleWatchlist(); }
      if (path === '/v1/trader/quote' && method === 'POST') { return await handleQuote(request, env); }
      if (path === '/v1/trader/signal' && method === 'POST') { return await handleSignal(request, env); }
      if (path === '/v1/trader/capital-call' && method === 'POST') { return await handleCapitalCall(request, env); }
      if (path === '/v1/trader/portfolio' && method === 'POST') { return await handleTraderPortfolio(request, env); }
      if (path === '/v1/trader/news' && method === 'GET') { return await handleTraderNews(env); }
      if (path === '/v1/trader/trade' && method === 'POST') { return await handleLogTrade(request, env); }
      if (path === '/v1/trader/history' && method === 'GET') { return await handleTradeHistory(url, env); }
      if (path === '/v1/trader/capital-tiers' && method === 'GET') { return handleCapitalTiers(); }
      ...
      if (path.match(/^\/v1\/hierarchy\/chain\/[^/]+$/) && method === 'GET') { const agentId = path.split('/v1/hierarchy/chain/')[1]; return handleChainOfCommand(agentId); }
      if (path.match(/^\/v1\/hierarchy\/reports\/[^/]+$/) && method === 'GET') { const agentId = path.split('/v1/hierarchy/reports/')[1]; return handleDirectReports(agentId); }
      if (path.match(/^\/v1\/hierarchy\/division\/[^/]+$/) && method === 'GET') { const divisionCode = path.split('/v1/hierarchy/division/')[1]; return handleDivisionHierarchyEndpoint(divisionCode); }

      // ── Slack (auth-protected read-only) ──
      if (path === '/v1/slack/channels' && method === 'GET') { return handleSlackChannels(); }
      if (path === '/v1/slack/apps' && method === 'GET') { return handleSlackApps(); }
      if (path === '/v1/slack/audit' && method === 'GET') { return handleSlackAudit(); }

      // ── Thinking Coach ──
      if (path === '/v1/thinking/frameworks' && method === 'GET') { return handleListThinkingFrameworks(url); }
      if (path.startsWith('/v1/thinking/frameworks/') && method === 'GET') { const frameworkId = path.split('/')[4]; return handleGetThinkingFramework(frameworkId); }
      if (path === '/v1/thinking/session' && method === 'POST') { return await handleThinkingSession(request, env, ctx); }
      if (path === '/v1/thinking/multi' && method === 'POST') { return await handleMultiFramework(request, env, ctx); }
      if (path === '/v1/thinking/learning-blueprint' && method === 'POST') { return await handleLearningBlueprint(request, env, ctx); }
      if (path === '/v1/thinking/daily-models' && method === 'POST') { return await handleDailyModels(request, env, ctx); }
      if (path === '/v1/thinking/pm-mastery' && method === 'POST') { return await handlePMMastery(request, env, ctx); }
      if (path === '/v1/thinking/cognitive-os' && method === 'POST') { return await handleCognitiveOS(request, env, ctx); }
      if (path === '/v1/thinking/life-architecture' && method === 'POST') { return await handleLifeArchitecture(request, env, ctx); }
      if (path === '/v1/thinking/time-leverage' && method === 'POST') { return await handleTimeLeverage(request, env, ctx); }
      if (path === '/v1/thinking/reprogram' && method === 'POST') { return await handleReprogram(request, env, ctx); }
      if (path === '/v1/thinking/dashboard' && method === 'GET') { return handleThinkingDashboard(); }

      // ── CEO Sovereign Directives ──
      if (path === '/v1/ceo/directive' && method === 'POST') { return await handleCeoDirective(request, env, ctx); }
      if (path === '/v1/ceo/operations-review' && method === 'POST') { return await handleOperationsReview(request, env, ctx); }
      if (path === '/v1/ceo/operating-state' && method === 'GET') { return handleOperatingState(); }
      if (path === '/v1/ceo/dashboard' && method === 'GET') { return handleCeoDashboard(); }
      ...
      if (path.match(/^\/v1\/orchestrator\/agents\/[^/]+\/health$/) && method === 'POST') { const agentKey = path.split('/')[4]; return handleOrchestratorAgentHealth(request, env, ctx, agentKey); }
      if (path.match(/^\/v1\/orchestrator\/agents\/[^/]+$/) && method === 'GET') { const agentKey = path.split('/')[4]; return handleOrchestratorAgentDetail(request, env, ctx, agentKey); }
      ...
      if (env.AUDIT_LOG) {
        ...
        ctx.waitUntil(env.AUDIT_LOG.put(key, JSON.stringify({ path, method, error: err.message, stack: err.stack, timestamp: new Date().toISOString() }), { expirationTtl: 86400 * 30 }));
      }
