...
describe('CEO Daily Standup', () => {
  ...
  it('fleet status shows 404 total units', () => {
    const standup = buildStandup();
    assert.equal(standup.fleetStatus.totalUnits, 404);
    assert.equal(standup.fleetStatus.active, 404);
    assert.equal(standup.fleetStatus.standby, 0);
    assert.equal(standup.fleetStatus.operationalReadiness, '100%');
  });

  it('includes all 12 divisions', () => {
    const standup = buildStandup();
    assert.equal(standup.divisions.length, 12);
    ...
    assert.ok(codes.includes('COOP'));
    assert.ok(codes.includes('CDX'));
  });

  it('division agent counts sum to 333', () => {
    const standup = buildStandup();
  ...
  it('generates text summary', () => {
    ...
    assert.ok(standup.summary.includes('FLEET: 404/404 ACTIVE'));
