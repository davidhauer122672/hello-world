...
async function classifyObjection(transcript) {
  ...
  try {
    const message = await client.messages.create({
      ...
      max_tokens: 150,
      temperature: 0.3,
      system: `You are an objection classifier for Coastal Key Property Management sales calls.
...
async function generateReframe(transcript, context = {}) {
  try {
    const message = await client.messages.create({
      ...
      max_tokens: 150,
      temperature: 0.3,
      system: `You are a real-time sales reframe assistant for Coastal Key Property Management.
