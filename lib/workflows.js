...
// ─────────────────────────────────────────────────────────────────
// Slack notification dispatch — all workflows report to Slack
// ─────────────────────────────────────────────────────────────────

const SLACK_WEBHOOK_URL = process.env.SLACK_WEBHOOK_URL;

async function notifySlack(channel, text, blocks) {
  if (!SLACK_WEBHOOK_URL) {
    console.warn('[workflow] SLACK_WEBHOOK_URL not configured — Slack notification skipped');
    return false;
  }
  try {
    const body = { text };
    ...
    console.error('[workflow] Slack notification failed:', err.message);
    return false;
  }
}
...
function logExecution(workflow, input, output, status) {
  ...
  // Dispatch Slack notification for every workflow execution
  const icon = status === 'success' ? ':white_check_mark:' : ':x:';
  const outputBrief = typeof output === 'string' ? output.substring(0, 200) : JSON.stringify(output).substring(0, 200);
  notifySlack(null, `${icon} *${workflow}* — ${status}\n${outputBrief}`).catch(() => {});
