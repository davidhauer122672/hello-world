 ...
 * Replaces all 7 Zapier workflows with native Node.js logic.
...
// ─────────────────────────────────────────────────────────────────
// Email transporter (shared with mailer.js and drip-engine.js)
// ─────────────────────────────────────────────────────────────────
...
const CEO_EMAIL = process.env.CEO_EMAIL || 'david@coastalkey-pm.com';
...
async function wf1_newLeadNurture(payload) {
  ...
  try {
    ...
    await transporter.sendMail({
      from: FROM,
      to: CEO_EMAIL,
      subject: `New Lead: ${leadName} — ${segment || 'Unclassified'}`,
      html: buildLeadAlertEmail(leadName, email, phone, segment, propertyAddress, propertyValue, source, battlePlan),
    });

    // Enroll in drip sequence (if drip engine available)
    try {
    ...
    return logExecution('WF-1', payload, `Lead alert sent to ${CEO_EMAIL}, drip enrolled`, 'success');
  } catch (err) {
    return logExecution('WF-1', payload, err.message, 'error');
  }
}
...
async function wf3_investorEscalation(payload) {
  ...
  try {
    ...
    await transporter.sendMail({
      from: FROM,
      to: CEO_EMAIL,
      subject: `INVESTOR ESCALATION: ${leadName} — ${propertyValue || '$5M+'}`,
    ...
    return logExecution('WF-3', payload, `Investor escalation sent for ${leadName}`, 'success');
  } catch (err) {
    return logExecution('WF-3', payload, err.message, 'error');
  }
}
...
async function wf5_videoBriefToProduction(payload) {
  ...
  try {
    ...
    for (const platform of platformList) {
      const brief = generateSocialBrief(
    ...
    await transporter.sendMail({
      from: FROM,
      to: CEO_EMAIL,
      subject: `Production Brief: ${videoTitle}`,
      html: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;">
    ...
    return logExecution('WF-5', payload, `Production brief assembled: ${thumbnail.id}, ${briefs.length} social briefs`, 'success');
  } catch (err) {
    return logExecution('WF-5', payload, err.message, 'error');
  }
}
