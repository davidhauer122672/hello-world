 ...
 * downstream actions: email, SMS, Claude AI, AI generation, record updates.
 *
 * WF-1: New Lead Nurture
 * WF-2: Social Approval → Claude AI Platform
 * WF-3: Investor Escalation
 * WF-4: Post Published → Airtable
...
// Actions: Prepare for publishing (via social-publisher), notify CEO
// ─────────────────────────────────────────────────────────────────
...
async function wf2_socialApproval(payload) {
  ...
  try {
    ...
    const mode = result.publishInstructions ? 'claude-ai' : 'approved';
...
const WORKFLOW_MAP = {
  ...
  'post-published': wf4_postPublished,
