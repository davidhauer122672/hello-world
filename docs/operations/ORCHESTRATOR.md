...
| POST | `/v1/orchestrator/dispatch` | Route event through Priority×Risk + Goal validation + HITL gate. Returns 200/202/409/422 |
...
- `RATE_LIMITS_CONFIG` — Per-agent RPM / daily / burst caps.
- `HITL_THRESHOLDS` — $5,000 single-transfer ceiling, P0-always, inter-LLC-always, insurance-always gates.
- `TRIGGER_ACTION_SEQUENCES` — 15 TAS (TAS-001…TAS-015) covering emergency maintenance, bill pay, HITL approval, lead enrichment, lease renewal, AR follow-up, and more.
- `classifyDispatch(event)` — Maps action string to priority, risk class, and assigned agent.
- `validateGoalAlignment(event)` — Verifies event goal alignment against G1–G4 catalogue.
- `evaluateHITL(event, classified)` — Returns `{required: bool, reason}` per threshold matrix.
- `routeDispatch(event)` — Full dispatch pipeline: classify → validate goals → HITL gate → `dispatched|hitl_pending|quarantined|errored`.
- `getOrchestratorFleetStatus()` — Returns live fleet array with counts and HITL threshold snapshot.
...
| [GOVERNANCE.md](../governance/GOVERNANCE.md) | Enterprise Governance and Standards Framework |
| [MISSION.md](../governance/MISSION.md) | Mission Statement and Operating Principles |
...
| [COASTAL-KEY-SOVEREIGN-GOVERNANCE.md](../governance/COASTAL-KEY-SOVEREIGN-GOVERNANCE.md) | R.E.S. Intelligence Co. Franchise and global expansion. |
