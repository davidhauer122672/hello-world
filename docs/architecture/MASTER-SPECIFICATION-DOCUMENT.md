...
├── th-sentinel-campaign/        # Retell AI campaign configs, 7 campaigns, prompt scripts
...
- **Retell Campaigns:** Campaign routing via `metadata.campaign_tag` on webhook (6 campaign types)
...
| Retell AI (ElevenLabs) | Voice campaigns — 7 campaign types, 2,400 daily capacity |
...
- **Speed-to-Lead:** Contact within 60 seconds via Retell AI
...
ANTHROPIC_API_KEY, AIRTABLE_API_KEY, WORKER_AUTH_TOKEN, SLACK_WEBHOOK_URL, SLACK_BOT_TOKEN, SLACK_SIGNING_SECRET, RETELL_API_KEY, CLOUDFLARE_API_TOKEN, CLOUDFLARE_ACCOUNT_ID, STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET, TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, GOOGLE_SERVICE_ACCOUNT_KEY, ADMIN_TOKEN
...
- Gateway: `GET /v1/health?deep=true` (Airtable, Anthropic, Retell, KV)
